...
from .crud import get_top_products, get_channel_activity, get_posting_trends,get_top_detected_objects
...
@app.get("/api/reports/top-detected-objects", response_model=list[TopProduct])
def top_detected_objects(limit: int = 10, db: Session = Depends(get_db)):
    return get_top_detected_objects(limit)
